...
import { getPaperExpertReview } from "#app/services/paper.server";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    if (res.data?.message === "No expert review found for this paper") {
      return redirect(`/paper/${paperId}/expert-review/form`);
    }
...
export const ExpertReviewPage = () => {
  ...
  const createdAt = formatDate(data?.expert_review?.created_at!);
  const updatedAt =
    data?.expert_review?.updated_at === data?.expert_review?.created_at
      ? "-"
      : formatDate(data?.expert_review?.updated_at!);
  return (
    <Stack>
      <Stack pr="md" pl="md">
        <Group justify="space-between">
          <Stack gap="xs">
            <Title order={4}>Expert Review</Title>
            <Group>
          ...
          <Button
            ...
            variant="light"
            component={Link}
            to={`/paper/${data?.expert_review?.paper_id}/expert-review/form`}
          >
            Edit
          </Button>
        </Group>
      ...
      <CDivider />
      <Stack pr="md" pl="md">
        <FormattedText content={data?.expert_review?.body!} />
      </Stack>
    </Stack>
  );
};
