...
import { getPaperLiteratureReview } from "#app/services/paper.server";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  try {
    const res = await getPaperLiteratureReview({ paperId, request });
    if (res.data?.message === "No literature review found for this paper") {
      return redirect(`/paper/${paperId}/literature-review/form`);
    }
...
export const LiteratureReviewPage = () => {
  ...
  const createdAt = formatDate(data?.literature_review?.created_at!);
  const updatedAt =
    data?.literature_review?.updated_at === data?.literature_review?.created_at
      ? "-"
      : formatDate(data?.literature_review?.updated_at!);
  return (
    <Stack>
      <Stack pr="md" pl="md">
        <Group justify="space-between">
          <Stack gap="xs">
            <Title order={4}>Literature Review</Title>
            <Group>
      ...
      <CDivider />
      <Stack pr="md" pl="md">
        <FormattedText content={data?.literature_review?.body!} />
      </Stack>
    </Stack>
  );
};
