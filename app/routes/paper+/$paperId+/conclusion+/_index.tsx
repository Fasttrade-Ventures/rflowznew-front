...
import { getPaperConclusion } from "#app/services/paper.server";
...
export const loader = async ({ request, params }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    if (res.data?.message === "No conclusion found for this paper") {
      return redirect(`/paper/${paperId}/conclusion/form`);
    }
...
export const ConclusionPage = () => {
  ...
  const createdAt = formatDate(data?.conclusion?.created_at!);
  const updatedAt =
    data?.conclusion?.updated_at === data?.conclusion?.created_at
      ? "-"
      : formatDate(data?.conclusion?.updated_at!);
  return (
    <Stack>
      <Stack pr="md" pl="md">
        <Group justify="space-between">
          <Stack gap="xs">
            <Title order={4}>Conclusion</Title>
            <Group>
          ...
          <Button
            ...
            variant="light"
            component={Link}
            to={`/paper/${data?.conclusion?.paper_id}/conclusion/form`}
          >
            Edit
          </Button>
        </Group>
      ...
      <CDivider />
      <Stack pr="md" pl="md">
        <FormattedText content={data?.conclusion?.body!} />
      </Stack>
    </Stack>
  );
};
